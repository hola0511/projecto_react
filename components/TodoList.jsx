      ...
      <div className="max-w-2xl mx-auto">
        {loading ? (
          <ul className="space-y-3">
            {Array.from({ length: 6 }).map((_, i) => (
              <li key={i} className="h-14 rounded-xl bg-zinc-800 animate-pulse" />
            ))}
          </ul>
        ) : todos.length === 0 ? (
            ...
            {todos.map((todo) => (
              <li
                ...
                className={`flex items-center gap-4 px-5 py-4 rounded-xl border transition-all duration-150
                  ${
                    todo.completed
                      ? "bg-emerald-950/40 border-emerald-800/40 hover:border-emerald-600/60"
                      : "bg-zinc-900 border-zinc-800 hover:border-zinc-600"
                  }`}
              >
                <span className={`text-lg shrink-0 ${todo.completed ? "text-emerald-400" : "text-red-400"}`}>
                  {todo.completed ? "✔" : "❌"}
                </span>
                ...
                <span className={`text-sm flex-1 capitalize ${todo.completed ? "text-zinc-400 line-through" : "text-zinc-200"}`}>
                  {todo.title}
                </span>

                <span className={`text-xs px-2 py-0.5 rounded-full font-medium shrink-0
                  ${todo.completed ? "bg-emerald-900/60 text-emerald-400" : "bg-zinc-800 text-zinc-500"}`}>
        ...
        {!loading && (
          <p className="text-center text-zinc-600 text-xs mt-8">
            {todos.length} tarea{todos.length !== 1 ? "s" : ""} mostrada{todos.length !== 1 ? "s" : ""}
          </p>
        )}
      </div>
    </main>
